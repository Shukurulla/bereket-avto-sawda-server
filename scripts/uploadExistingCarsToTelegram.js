...
const uploadExistingCars = async () => {
  try {
    ...
    if (!bot) {
      console.error(
        "❌ Bot ishlamadi. .env faylida TELEGRAM_BOT_TOKEN va TELEGRAM_CHANNEL_ID ni tekshiring."
      );
      process.exit(1);
    }
    ...
    const cars = await Car.find({
      $or: [{ telegramPostId: { $exists: false } }, { telegramPostId: null }],
    ...
    for (let i = 0; i < cars.length; i++) {
      const car = cars[i];
      console.log(
        `\n[${i + 1}/${cars.length}] Yuklanmoqda: ${car.brand} ${car.model} (${
          car.year
        })`
      );
